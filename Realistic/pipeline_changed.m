function videoTo3DModelFromEnhanced()
    ...
    for i = 2:numFrames
        ...
        if size(matchedPoints1, 1) >= 6
            ...
            %literally the only thing I changed is made them into variables
            %then multiplied them instead of just multiplying the straight
            %on
            prevPose = poses(vSet, i-1);
            prevOrient = prevPose.Orientation{1};
            prevLoc = prevPose.Location{1};
        
            absOrient = prevOrient * relOrient;
            absLoc = prevLoc + relLoc * prevOrient;
